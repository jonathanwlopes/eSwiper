...
export const swiperStructure = (swiperContainer, configs) => {
  const { scrollbar, arrows, pagination, lazy, isZoom } = configs
  ...
  _slides.forEach((_slide) => {
    ...
    if (isZoom && desktopWidth) {
      ...
      _slide.appendChild(_wrapperZoom)
...
export const thumbsGenerate = (swiper, configs) => {
  ...
  const { slidesPerView = 1, spaceBetween = 2, direction, loop, arrows, slideToClickedSlide } = thumbsConfigs

  console.log(slideToClickedSlide)
  const _thumbsContainer = thumbsStructure(configs)

  let swiperThumbs = new Swiper(_thumbsContainer, {
    direction,
    loop,
    arrows,
    spaceBetween,
